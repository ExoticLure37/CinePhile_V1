...
const editComment = async (req, res) => {
    try {
        ...
        if (!text) {
            return res.status(400).json({ error: true, message: "New comment text is required." });
        }
        ...
        // Authorization
        if (comment.author.toString() !== req.user._id.toString()) {
            return res.status(403).json({ error: true, message: "Not authorized to edit this comment." });
        }
