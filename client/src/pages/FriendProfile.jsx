  ...
  const getFriendProfile = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE_URL}/user/getProfile/${friendId}`,
      );
    ...
    try {
      if (isFriend === "Unfriend") {
        const res = await axios.patch(
          `${process.env.REACT_APP_BACKEND_BASE_URL}/user/removeFriend`,
          {
            friendId: friendId,
          },
          {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#141414] text-white">
      ...
      <div className="max-w-4xl mx-auto flex-col p-6">
        <h1 className="mb-10 text-4xl mx-auto font-bold text-center  text-white">
          Profile
        </h1>
